...
음수 가중치가 있는 그래프는 다익스트라로 정확한 결과를 얻을 수 없습니다. 그래서 사용 되는 것이 벨만-포드(Bellman-Ford) 알고리즘인데, 정점의 개수가 V인 그래프의 모든 에지에 대해 다익스트라의 그리디 선택 방법을 (V-1)번 반복하여 점진적으로 최단 거리를 찾습니다.
