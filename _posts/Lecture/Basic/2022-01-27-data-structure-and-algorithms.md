...
분할 정복의 예시로 "병합 정렬" 을 들 수 있는데, 배열의 크기가 충분히 작아질 때까지 같은 크기인 2개의 부분배열로 **분할** 하고 부분 배열을 정렬하여 **정복** 하고 정렬된 배열들을 하나로 병합함으로써 **결합** 합니다. 하지만 보통 결합과 동시에 정렬하기 때문에 정복과 결합이 동시에 이루어진다고 볼 수 있습니다.

## 동적계획법

문제중 중복으로 풀이되는 문제의 풀이를 기억해서(Memoization) 재활용하거나 또는 이전의 부분문제의 풀이를 이용해서 다음 문제의 답을 구하는(Tabulation) 패러다임
